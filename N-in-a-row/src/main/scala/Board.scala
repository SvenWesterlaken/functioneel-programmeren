...
class Board(val rows: Int, val columns: Int) {
  ...
  var board: Array[Array[Player]] = Array.fill[Player](rows,columns) {Player.NoPlayer}
  ...
  def sinkChecker(column: Int, player: Player): Unit = {
    ...
    while (isPositionInBoard(row, column) && getCheckerAtPosition(row+1, column) == Player.NoPlayer ) {
      row += 1
    }
  ...
  override def toString: String = {
    ...
    for (row <- 1 to rows) {
      for (column <- 1 to columns) {
        val player = getCheckerAtPosition(row, column)
        var str = "0"
